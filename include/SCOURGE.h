...
public:
	void LoadPluginData()
	{
		...
		if ((spellForm = dataHandler->LookupForm(0x1, "SCOURGE.esp")) != nullptr) {
			REX::INFO("Loaded SCOURGE_SPEL");
		} else {
			REX::INFO("Unable to load SCOURGE_SPEL");
...
private:
	...
	void GetActorValues(RE::TESDataHandler* a_dataHandler)
	{
		...
		{
			REX::INFO("Loaded ActorValues");
		} else {
			REX::INFO("Unable to load ActorValues");
			loadedCorrectly = false;
		}
	}
	...
	void GetKeywords(RE::TESDataHandler* a_dataHandler)
	{
		...
		{
			REX::INFO("Loaded Keywords");
		} else {
			REX::INFO("Unable to load Keywords");
			loadedCorrectly = false;
		}
	}
	...
	void GetGlobalMults(RE::TESDataHandler* a_dataHandler) {
		...
		{
			REX::INFO("Loaded global mults");
		} else {
			REX::INFO("Unable to load global mults");
			loadedCorrectly = false;
		}
	}
	...
	void LinkPluginRecords()
	{
		...
		if (pluginRecords.size() != AV_COUNT) {
			REX::INFO("!!! AV_COUNT does not match the size of pluginRecords !!!");
		}

		REX::INFO("Linked plugin records");
	}
