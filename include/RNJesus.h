...
public:
	...
	T GaussianDistribution(T a_minValue, T a_maxValue)
	{
		...
		if (a_maxValue <= a_minValue)
			return a_minValue; // or fallback safe value

		T mean = (a_maxValue + a_minValue) / T(2.0);  // stay safe kids
		T stddev = (a_maxValue - mean) / T(3.3333333333333333);

		if (stddev <= std::numeric_limits<T>::epsilon() || !std::isfinite(stddev)) {
			return mean;
		}
