...
public:
	void LoadConfigs() {
		...
		if (fileList.empty()) {
			REX::INFO("No YAML files detected");
			return;
		} else {
			if (fileList.size() != 1) {
				REX::INFO("Found {} files", fileList.size());
			} else {
				REX::INFO("Found {} file", fileList.size());
			}
			REX::INFO("---------------------------------------------------------------------");
		}
		...
		for (std::string currentFile : fileList) {
			...
			fuck_around {
				...
				REX::INFO("Loaded file: \"{}\"", currentFile);
			} find_out (const YAML::Exception& except) {
				REX::INFO("Error loading YAML file: {}", except.what());
				continue;
			}

			if (yamlConfig.IsNull()) {
				REX::INFO("The file is empty: \"{}\"", currentFile);
				continue;
			}
			...
			for (const auto& node : yamlConfig) {
				...
				REX::INFO("{}: \"{}\" from \"{}\"", filterCount, filterStruct.filterName, currentFile);
				REX::INFO("Global string:");
				REX::INFO("   {}", filterStruct.globalString);
				...
				if (filterStruct.npcList.empty() && filterStruct.raceList.empty() && filterStruct.factionList.empty() && filterStruct.keywordList.empty()) {  //&& filterStruct.gender == NPC_GENDER::any) {
					REX::INFO("The non-gender conditions inside the filter {} are invalid or empty. Make sure that all Forms or EditorIDs are correct. If you're trying to distribute to all NPCs use the keyword ActorTypeNPC~13794~Fallout4.esm", filterStruct.filterName);
					continue;
				}
				...
				for (auto item : filterStruct.npcList) {
					REX::INFO("   {}", uint32ToHexString(item));
				}

				REX::INFO("Races:");
				for (auto item : filterStruct.raceList) {
					REX::INFO("   {}", item->GetFormEditorID());
				}

				REX::INFO("Factions:");
				for (auto item : filterStruct.factionList) {
					REX::INFO("   {}", item->As<RE::TESForm>()->GetFormEditorID());
				}
				...
				REX::INFO("Gender:");
				REX::INFO("   Male: {}", filterStruct.gender.male);
				REX::INFO("   Female: {}", filterStruct.gender.female);

				REX::INFO("---------------------------------------------------------------------");
			}
		}

		REX::INFO("Finished reading files");
		REX::INFO("---------------------------------------------------------------------");
	}
		...
		if (a_parent.IsNull() == false) {
			...
			for (const auto& child : a_parent) {
				...
				if (editorID.empty() == false) {
					...
					if (formPtr) {
						REX::INFO("Loaded NPC by EditorID");
	...
	void GetGender(const YAML::Node& a_parent, FilterData& a_filterStruct)
	{
		if (a_parent["Male"]) {
			...
			if (ToLower(male) == "true" || ToLower(male) == "t" || ToLower(male) == "1") {
				...
				REX::INFO("{} is not a valid setting for Gender. Defaulting to false", male);
				a_filterStruct.gender.male = false;
			}
		}
		...
		if (a_parent["Female"]) {
			...
			if (ToLower(female) == "true" || ToLower(female) == "t" || ToLower(female) == "1") {
				...
				REX::INFO("{} is not a valid setting for Gender. Defaulting to false", female);
				a_filterStruct.gender.female = false;
			}
		}
	}
