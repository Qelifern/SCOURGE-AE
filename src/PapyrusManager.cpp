...
namespace Papyrus
{
	...
	{
		if (!a_actor || a_statArray.empty()) return;
		...
		for (int i = 1; i < a_statArray.size(); i++) {
			...
			if (i < 5) {
				...
				a_actor->ModActorValue(RE::ACTOR_VALUE_MODIFIER::kPermanent, *pluginData.pluginRecords[i].actorValue, randomisedValue - trashValue);
			}
